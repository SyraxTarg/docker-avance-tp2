...
async def verify_recaptcha(token: str) -> bool:
    ...
    if not secret:
        raise RuntimeError("RECAPTCHA_SECRET_KEY not set")
...
async def login_for_access_token(
    ...
    if failed_login:
        # Reset après 60s
        if datetime.now() - failed_login.last_attempt > timedelta(minutes=3):
    ...
    is_human = await verify_recaptcha(user_login.recaptcha_token)
    if not is_human:
        raise RecaptchaFailed(
            status_code=400,
            detail="Échec de vérification reCAPTCHA. Veuillez réessayer.",
        )


    # Générer les tokens
    refresh_expire_days = int(os.getenv("REFRESH_TOKEN_EXPIRE_DAYS_REMEMBER_ME")) if user_login.remember_me else int(os.getenv("REFRESH_TOKEN_EXPIRE_DAYS"))
    ...
    refresh_token = create_refresh_token(data={"sub": user.email}, expires_delta=timedelta(days=refresh_expire_days))
    ...
    response.set_cookie(
        ...
        access_token,
        httponly=True,
        secure=False,  # À mettre à True en prod avec HTTPS
        samesite="Strict",
        max_age=int(timedelta(minutes=int(os.getenv("ACCESS_TOKEN_EXPIRE_MINUTES"))).total_seconds()),
    )
...
async def refresh_token(
    ...
    if authorization is None:
        raise NoRefreshTokenError(status_code=status.HTTP_400_BAD_REQUEST, detail="No Authorization header provided")

    parts = authorization.split()
    if len(parts) != 2 or parts[0].lower() != "bearer":
        raise InvalidTokenError(status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid Authorization header format")
    ...
    try:
        ...
        email = payload.get("sub")
        if email is None:
            raise InvalidTokenError(status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid Token")

        user = user_service.get_user_by_email(db, email)
        if user is None:
            raise UserNotFoundError(status_code=status.HTTP_404_NOT_FOUND, detail="User not found")
        ...
        # Optionnel : tu peux mettre le access_token dans un cookie HttpOnly si tu veux
        response.set_cookie(
            "access_token",
            access_token,
            httponly=True,
        ...
        raise InvalidTokenError(status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid Refresh Token")
...
def send_mail_for_password_reset(db: Session, user: User) -> None:
    ...
    verification_url = f"{HOST}/auth/reset?t={reset_token}"
    html_content = template.render(
        first_name=profile.first_name,
        token=user.verification_token,
        url=verification_url
