...
import { updateProfile } from "../../profiles/me/server_actions/update_profile";
...
const profileSchema = z.object({
  first_name: z.string().min(1, "Le prénom est requis").max(50, "Prénom trop long"),
  last_name: z.string().min(1, "Le nom est requis").max(50, "Nom trop long"),
  phone: z.string()
    .min(10, "Le numéro doit contenir au moins 10 chiffres")
    .max(15, "Numéro trop long")
    .regex(/^\+?\d+$/, "Numéro de téléphone invalide (doit contenir uniquement des chiffres et un + optionnel)"),
});
...
export default function UpdateProfileModal({ isOpen, onClose }: UpdateProfileModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      let uploadedPhotoUrl = "/pfps/default.jpg";
      if (photo) {
        uploadedPhotoUrl = await uploadPhoto(photo);
      }
      await updateProfile(first_name, last_name, phone, uploadedPhotoUrl);
      await refetchUser();
      router.refresh();
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center">
      <div className="w-full max-w-sm sm:max-w-md max-h-[90vh] overflow-y-auto bg-white rounded-xl shadow-xl p-6 m-4">
        <h2 className="text-xl font-bold text-center text-[#0F172A] mb-6">
          Modifier mon profil
        </h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 mt-4">
          ...
          <div className="flex justify-end space-x-3 pt-4">
            ...
            <button
              type="submit"
              className="px-4 py-2 rounded bg-[#4338CA] hover:bg-[#6366F1] text-white transition-colors cursor-pointer"
            >
              Modifier
            </button>
          </div>
