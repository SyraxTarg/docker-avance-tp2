...
import { createSignaledUser } from "@/app/profiles/[profile_id]/server_actions/create_signaled_user";
...
export default function SignalUserModal({ isOpen, user_id, onClose, reasons }: SignalUserModalProps) {
  ...
  const onSignal = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await createSignaledUser(selectedReason.id, user_id);
