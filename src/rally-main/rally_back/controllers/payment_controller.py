...
def refund_payment_controller(
    ...
    Checks:
    - payment exists
    - payment belongs to current_user (or is authorized)
    - refund has not already been issued

    Returns:
        dict: Confirmation of refund
    """
    payment = payment_service.get_payment(db, event_id, current_user_id)
    print(payment.buyer_id, payment.buyer_email)
    if not payment:
    ...
    try:
        ...
        payment_service.send_refund_confirmation_to_buyer(db, payment.id)
