...
def get_signaled_user_by_user_signaled_id(
    db: Session, user_signaled_id: int
) -> list[SignaledUser]:
    """Retrieve signaled users by the ID of the user who was reported."""
    return (
        db.query(SignaledUser)
        .filter(SignaledUser.user_signaled_id == user_signaled_id)
        .all()
    )


def get_signaled_user_by_id(
    db: Session, signaled_user_id: int
) -> Optional[SignaledUser]:
...
def get_signaled_users_filters(
    ...
    if date is not None:
        query = query.filter(func.date(SignaledUser.created_at) <= date.date())
    ...
    if email_user is not None:
        query = query.join(user_alias, SignaledUser.user).filter(
            user_alias.email == email_user
        )

    if email_signaled_user is not None:
        query = query.join(signaled_user_alias, SignaledUser.user_signaled).filter(
            signaled_user_alias.email == email_signaled_user
        )

    return (
        query.order_by(SignaledUser.created_at.desc()).offset(offset).limit(limit).all()
    )
