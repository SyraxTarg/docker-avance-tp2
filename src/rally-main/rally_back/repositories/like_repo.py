...
def get_like_by_profile_and_event(db: Session, profile_id: int, event_id: int) -> Like:
    ...
    return (
        db.query(Like)
        .filter(Like.profile_id == profile_id, Like.event_id == event_id)
        .first()
    )
...
def get_likes_count_from_event(db: Session, event_id: int) -> int:
    """
    This function is used to fetch the count of likes from db according to their events.
    """
    return db.query(Like).filter(Like.event_id == event_id).count()
