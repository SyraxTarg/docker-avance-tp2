...
def get_signaled_comments_by_comment_id(
    db: Session, comment_id: int
) -> list[SignaledComment]:
    """Retrieve signaled comments by comment ID."""
    return (
        db.query(SignaledComment).filter(SignaledComment.comment_id == comment_id).all()
    )


def get_signaled_comment_by_id(
    db: Session, signaled_comment_id: int
) -> SignaledComment:
    """Retrieve a signaled comment by its ID."""
    return (
        db.query(SignaledComment)
        .filter(SignaledComment.id == signaled_comment_id)
        .first()
    )
...
def get_signaled_comment_by_reason_id(
    db: Session, reason_id: int
) -> list[SignaledComment]:
    """Retrieve signaled comments by reason ID."""
    return (
        db.query(SignaledComment).filter(SignaledComment.reason_id == reason_id).all()
    )
...
def get_signaled_comments_filters(
    ...
    if date is not None:
        query = query.filter(func.date(SignaledComment.created_at) <= date.date())
    ...
    if email_user is not None:
        query = query.join(user_alias, SignaledComment.user).filter(
            user_alias.email == email_user
        )

    if email_comment_user is not None:
        query = (
            query.join(comment_alias, SignaledComment.comment_signaled)
    ...
    return (
        query.order_by(SignaledComment.created_at.desc())
        .offset(offset)
        .limit(limit)
        .all()
    )
