...
def signal_user(
    ...
    return signaled_user_controller.create_signaled_user_by_current(
        db, signaled_user, current_user
    )


# 🔹 5. Mettre à jour le statut d'un signalement
@router.patch(
    "/{signaled_user_id}", response_model=SignaledUserSchemaResponse, status_code=200
)
def update_status_signaled_user(
    ...
    return signaled_user_controller.update_status_signaled_user(
...
def delete_signaled_user(
    ...
    return signaled_user_controller.delete_signaled_user(
        db, signaled_user_id, current_user, ban
    )
