from fastapi import Depends, Cookie, Request, APIRouter, Query, Header
...
@router.post("/verify")
async def verify_jwt(user: UserResponse = Depends(authent_controller.verify_jwt)):
    return user
...
async def verify_token(
    db: Session = Depends(get_db), user_email: str = Query, token: int = Query
):
...
async def send_token(db: Session = Depends(get_db), user_email: str = Query):
...
async def generate_link_to_reset_pwd(
    db: Session = Depends(get_db), user_email: str = Query
):
...
async def reset_pwd(
    reset_password_schema: ResetPasswordSchema, db: Session = Depends(get_db)
):
