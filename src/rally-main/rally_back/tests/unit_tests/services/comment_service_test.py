...
def test_create_comment(
    ...
    result = comment_service.comment_event(
        db=mock_db_session, profile_id=1, event_id=1, content="Mon super commentaire"
    )
...
def test_get_comments(mock_comment, mock_db_session, mock_get_all_comments):
    ...
    # Act
    result = comment_service.get_all_comments(
        mock_db_session, 1, 2, "email", "event_email", "2025-06-02", "search"
    )
...
def test_get_comment_by_event_id(
    mock_comment, mock_db_session, mock_get_comment_by_event_id
):
    # Arrange
    ...
    result = comment_service.get_comment_by_event(
        db=mock_db_session, event_id=1, offset=1, limit=2
    )
...
def test_get_comment_from_profile(
    mock_comment, mock_db_session, mock_get_comments_by_profile_id
):
    # Arrange
    ...
    result = comment_service.get_comment_from_profile(
        db=mock_db_session, profile_id=1, offset=1, limit=2
    )
