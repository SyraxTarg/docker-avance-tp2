...
def get_reasons(db: Session, limit: Optional[int], offset: Optional[int])->list[Reason]:
    """used to fetch all reasons"""
    return reason_repo.get_all_reasons(db, limit, offset)

def get_reasons_total_count(db: Session)->int:
    """used to fetch all reasons total count"""
    return reason_repo.get_all_reasons_total_count(db)
