...
from errors import SignaledUserNotFound, BadRoleError, UserNotFoundError, RoleNotFound


def create_signaled_user(
    db: Session, signaled_user_id: int, by_user_id: int, reason_id: int
) -> SignaledUser:
...
def get_signaled_user_by_id(db: Session, user_signaled_id: int) -> list[SignaledUser]:
    ...
    return signaled_user_repo.get_signaled_user_by_user_signaled_id(
        db, user_signaled_id
    )
...
def get_signaled_user(db: Session, signaled_user_id: int) -> Optional[SignaledUser]:
    ...
    if not signaled_user:
        raise SignaledUserNotFound(
            status_code=status.HTTP_404_NOT_FOUND, detail="Signaled user not found"
        )
...
def update_status_signaled_user(
    db: Session, signaled_user_id: int, status_: str
) -> SignaledUser:
