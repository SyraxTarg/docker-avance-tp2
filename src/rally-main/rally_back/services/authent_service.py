...
def register_user(
    ...
    return user_service.create_user(
        db, email, password, phone_number, first_name, last_name, photo
    )
...
async def login_for_access_token(
    request: Request, user_login: UserAuth, db: Session = Depends(get_db)
) -> JSONResponse:
    """used to login user"""
    ...
    # Générer les tokens
    refresh_expire_days = (
        int(os.getenv("REFRESH_TOKEN_EXPIRE_DAYS_REMEMBER_ME"))
        if user_login.remember_me
        else int(os.getenv("REFRESH_TOKEN_EXPIRE_DAYS"))
    )
    ...
    refresh_token = create_refresh_token(
        data={"sub": user.email}, expires_delta=timedelta(days=refresh_expire_days)
    )
    ...
    response.set_cookie(
        ...
        == "production",  # À mettre à True en prod avec HTTPS
        samesite="Strict",
        max_age=int(
            timedelta(
...
def get_connected_user(
    ...
    if not authorization and not access_token:
        raise InvalidTokenError(
            status_code=401, detail="Authorization header or cookie is required"
        )
...
def get_current_user(
    db: Session, authorization: str = Header(None), access_token: str = Cookie(None)
) -> User:
    """Get current user, checking token from Authorization header or cookie"""

    if not authorization and not access_token:
        raise InvalidTokenError(
            status_code=401, detail="Authorization header or cookie required"
        )
    ...
    try:
        payload = jwt.decode(
            token_to_use, os.getenv("SECRET_KEY"), algorithms=[os.getenv("ALGORITHM")]
        )
        email = payload.get("sub")
        if not email:
            raise InvalidTokenError(status_code=401, detail="Invalid Token")
...
def get_current_admin(
    ...
    if not authorization and not access_token:
        raise InvalidTokenError(
            status_code=401, detail="Both Authorization header and cookie are required"
        )
...
def get_current_super_admin(
    ...
    try:
        ...
        if (
            ...
            != role_service.get_role_by_id(db, user.role_id).role
        ):
            raise BadRoleError(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="You do not have the rights to access this ressource",
...
def get_super_admin_or_admin(
    ...
    try:
        ...
        if role_service.get_role_by_id(db, user.role_id).role not in (
            RoleEnum.ROLE_SUPER_ADMIN,
            RoleEnum.ROLE_ADMIN,
        ):
            raise BadRoleError(
...
async def refresh_token(db: Session, authorization: str = Header(None)) -> JSONResponse:
...
def send_mail_for_email_verif(db: Session, user: User) -> None:
    ...
    verification_url = f"{HOST}/auth/verification"
...
def reset_pwd(
    ...
    if new_password != confirm_password:
        raise DifferentPasswordsError(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Different passwords"
        )
    user = user_service.get_user_by_email(db, email)
    if not user:
        raise UserNotFoundError(
