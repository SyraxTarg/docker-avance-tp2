...
def create_signaled_comment(
    db: Session, comment_id: int, user_id: int, reason_id: int
) -> SignaledComment:
...
def get_signaled_comment_by_comment_id(
...
def get_signaled_comment_by_id(
    ...
    signaled_comment = signaled_comment_repo.get_signaled_comment_by_id(
        db, signaled_comment_id
    )
    if not signaled_comment:
        raise SignaledCommentNotFound(
            status_code=status.HTTP_404_NOT_FOUND, detail="Ressource not found"
        )
...
def get_signaled_comment_by_user_by_id(db: Session, user_id: int) -> SignaledComment:
    ...
    signaled_comment = signaled_comment_repo.get_signaled_comment_by_user_id(
        db, user_id
    )
    if not signaled_comment:
        raise SignaledCommentNotFound(
...
def update_status_signaled_comment(
    db: Session, signaled_comment_id: int, status_: str
) -> SignaledComment:
