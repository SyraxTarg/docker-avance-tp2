...
def get_types(db: Session, limit: Optional[int], offset: Optional[int]) -> list[Type]:
    """used to get all types"""
    return type_repo.get_types(db, limit, offset)


def get_types_total_count(db: Session) -> int:
    """used to get the count of all types"""
    return type_repo.get_types_total_count(db)
