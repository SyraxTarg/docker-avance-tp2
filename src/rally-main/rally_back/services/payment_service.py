...
def get_payment(db: Session, event_id: int, profile_id: int) -> Payment:
    """used to fetch a payment from db by its profile id and event id"""
...
def send_refund_notification(db: Session, payment_id: int) -> bool:
    """Send refund notification email to the organizer based on refunded payment."""
    ...
    if payment.status != "refunded":
        raise ValueError("Payment is not refunded")
    ...
    template_refund = env.get_template("refund.html")
    refund_content = template_refund.render(
        refund={
            "organizer_email": payment.organizer_email,
            "event_title": payment.event_title,
            "buyer_email": payment.buyer_email,
            "stripe_payment_intent_id": payment.stripe_payment_intent_id,
            "amount": int(payment.amount * 100),  # montant en centimes
        }
    )
...
def send_refund_confirmation_to_buyer(db: Session, payment_id: int) -> bool:
    """Envoie une confirmation de remboursement à l'acheteur"""
    ...
    template_buyer_refund = env.get_template("refunded.html")
    refund_content = template_buyer_refund.render(
        refund={
            "buyer_first_name": getattr(buyer, "first_name", None),
            "buyer_email": payment.buyer_email,
            "organizer_email": payment.organizer_email,
            "event_title": payment.event_title,
            "stripe_payment_intent_id": payment.stripe_payment_intent_id,
            "amount": int(payment.brut_amount * 100),
        }
    )
...
def refund_full_payment(payment_intent_id: str):
    try:
        ...
        raise PaymentError(status_code=400, detail=e) from e
