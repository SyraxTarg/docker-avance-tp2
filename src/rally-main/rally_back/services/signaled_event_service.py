...
def create_signaled_event(
    db: Session, event_id: int, user_id: int, reason_id: int
) -> SignaledEvent:
...
def get_signaled_event_by_id(db: Session, signaled_event_id: int) -> SignaledEvent:
    ...
    if not signaled_event:
        raise SignaledEventNotFound(
            status_code=status.HTTP_404_NOT_FOUND, detail="Signaled event not found"
        )
...
def get_signaled_events_by_filters_total_count(
    db: Session,
    date: Optional[datetime],
    reason_id: Optional[int],
    user_id: Optional[int],
    ...
    """used to get the count of signaled events according to given filters"""
    return signaled_event_repo.get_signaled_events_filters_total_count(
        db,
        date,
        reason_id,
        user_id,
