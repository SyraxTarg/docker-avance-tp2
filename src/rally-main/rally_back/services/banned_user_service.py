...
def create_banned_user(
    db: Session, banned_email: str, banned_by_email: str
) -> BannedUser:
...
def get_banned_user_total_count(db: Session) -> Optional[BannedUser]:
    """used to fetch banned user by email"""
    return banned_user_repo.get_all_banned_emails_total_count(db)
...
def delete_banned_user_by_email(db: Session, email: str) -> bool:
    ...
    if not banned_user:
        raise BannedUserNotFoundError(
            status_code=status.HTTP_404_NOT_FOUND, detail="Banned user not found"
        )
